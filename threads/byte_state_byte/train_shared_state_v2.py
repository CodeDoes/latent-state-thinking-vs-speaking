"""Training script for shared-state architecture v2.

Three RWKV components: encoder-rwkv → patch-rwkv → decoder-rwkv
Decoder state feeds back to encoder for recurrent loop.
"""
...
from threads.byte_state_byte.shared_state_model_v2 import SharedStateModelV2
...
def generate_sample(model, prompt_text: str, max_new_tokens: int = 80, max_len: int = 256):
    ...
    for _ in range(max_new_tokens):
        try:
            ...
            byte_state, _ = model.byte_model(x)
            final_byte_state = byte_state[:, -1, :]
            
            # Process through patch-model
            corrected_state, direction = model.patch_model(final_byte_state)
            
            # Generate next byte using decoder
            logits, _ = model.decoder(corrected_state, direction)
...
def main():
    ...
    ap.add_argument('--exp_id', default='shared_state_v2_001')
    ...
    print(f"  byte_model (encoder-rwkv): {count_params(model.byte_model):,}")
    print(f"  patch_model (patch-rwkv): {count_params(model.patch_model):,}")
    print(f"  decoder (decoder-rwkv): {count_params(model.decoder):,}")
    ...
    config = {
        ...
        "lr_schedule": "cosine",
        "n_params": n_params,
        "architecture": "encoder-rwkv → patch-rwkv → decoder-rwkv (decoder state feeds back to encoder)",
    }
