"""Train step 2: BLT with transformer encoder/decoder, RWKV patch mixer.

Mirror of step 3 (RWKV byte enc/dec, transformer patch): same 2x2 grid
of {byte, patch} × {transformer, RWKV}. Tests if RWKV handles the
coarser patch-level stream better than attention.

Matched setup to step 1 and step 3 for comparison.
"""
...
from threads.blt.blt_trans_enc_rwkv_patch import BLT_TransEncDec_RWKVPatch
...
def main():
    ...
    ap.add_argument('--exp_id', default='blt_trans_enc_rwkv_patch_001')
    ...
    # Experiment dir
    exp_dir = Path(f"threads/blt/experiments/{args.exp_id}")
    exp_dir.mkdir(parents=True, exist_ok=True)
    config = {
        "model": "blt_trans_enc_rwkv_patch",
    ...
    for step in range(args.steps):
        ...
        if step % args.log_every == 0 or step == args.steps - 1:
            ...
            print(f"step {step:4d} | loss {loss_val:.4f} (avg {avg_loss:.4f}) | {bytes_per_sec:.0f} bytes/s | {elapsed:.1f}s")
