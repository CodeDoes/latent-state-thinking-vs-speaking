...
| `exp_t09c_equalparams_2026-07-13` (equal param budget, both ~9.98M) | 2026-07-13 | T09,T08,fairness | lat **0.649** (stable) vs base **~0.16** (UNSTABLE 0.23/0.32/0.08/0.22/0.17/0.16 over 6 ep; run timed out ep6/8); baseline GRU@9.98M can't train on 450 tiny worlds -> latent wins decisively at equal capacity; suggests latent SCALING is more effective than scaling a GRU AR | ✅ done (partial) |
...
5. **T09c DONE (partial, 6/8 ep)** — user required EQUAL param counts. Auto-sized the baseline GRU to match the latent (both ~9.98M via `_match_baseline_params`). Result: latent **0.649 (stable)** vs baseline **~0.16 (unstable/collapsing)**. At equal capacity the latent wins decisively — and crucially, *scaling the baseline up* (399K→9.98M) made it *worse* (0.626→0.16) while scaling the latent helped. This suggests the latent architecture **scales better** than a GRU AR. Caveat: the 9.98M GRU baseline may just be hard to optimize (lr/instability), so a transformer-AR baseline of equal size would be a fairer test of the AR architecture per se.
