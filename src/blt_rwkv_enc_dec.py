"""BLT with RWKV encoder and decoder, transformer patch mixer.

Step 2 of the progression: BLT dual-stream structure, but encoder and
decoder use RWKV blocks instead of transformer blocks. Patch mixer stays
as causal transformer blocks for now.

Architecture:
    bytes -> embed -> [RWKVBlock × N] -> h_byte
                                    | (mean_pool every k bytes)
                                    v
                    [CausalTransformerBlock × M] -> h_patch
                                    | (repeat to byte positions)
                                    v
                h_byte + h_patch_per_byte -> LayerNorm -> linear -> logits

Why this step: isolate the effect of RWKV vs transformer in the
byte-level encoder/decoder while keeping the patch-level processing
fixed as transformer. Proves RWKV can handle the byte stream.
"""
...
from src.blt_pure import CausalTransformerBlock, broadcast_patches
...
class BLT_RWKV_EncoderDecoder(nn.Module):
    """BLT with RWKV encoder/decoder, transformer patch mixer.
