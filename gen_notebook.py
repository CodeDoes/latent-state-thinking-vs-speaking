#!/usr/bin/env python3
"""Generate notebook.ipynb with updated architecture.

All models now produce [batch, seq_len, vocab_size] for fair comparison.
...
nb = {
    "cells": [
        md(["# Hybrid Latent-State Language Model\n\n",
        ...
        code([
            ...
            "    def __init__(self, vs, ds=256, dm=256, nsl=2, ls=4, te=4, dr=0.1, selective=True):\n",
            "        super().__init__()\n",
        ...
        code([
            ...
            "for c in EXPS:\n",
            "    exp_id = c['exp_id']\n",
            "    metrics_file = f'experiments/{exp_id}/metrics.json'\n",
            "    try:\n",
            "        with open(metrics_file, 'r') as f:\n",
            "            exp_metrics[exp_id] = json.load(f)\n",
            "    except:\n",
            "        print(f'Warning: Could not load {metrics_file}')\n",
            "\n",
            ...
            "        plt.plot(metrics['train_loss'], label=exp_id, linewidth=2)\n",
            ...
            "        plt.plot(metrics['val_loss'], label=exp_id, linewidth=2)\n",
            "plt.xlabel('Epoch', fontsize=12)\n",
            ...
            "for c in EXPS:\n",
            "    exp_id = c['exp_id']\n",
            "    if exp_id in exp_metrics and 'val_loss' in exp_metrics[exp_id]:\n",
            "        exp_ids.append(exp_id)\n",
            "        final_val_losses.append(exp_metrics[exp_id]['val_loss'][-1])\n",
            "\n",
            "colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']\n",
            "bars = plt.bar(exp_ids, final_val_losses, color=colors[:len(exp_ids)], alpha=0.7, edgecolor='black')\n",
            "\n",
            ...
            "print('\\n📊 Model Performance:')\n",
            "print('-' * 60)\n",
            "print(f\"{'Exp':<10} {'Model':<20} {'Val Loss':<12} {'Params':<12}\")\n",
            "print('-' * 60)\n",
            "\n",
            "best_exp = None\n",
            ...
            "        r = results[exp_id]\n",
            "        print(f\"{exp_id:<10} {r['model']:<20} {r['val_loss']:<12.4f} {r['params']:<12,}\")\n",
            ...
            "print('-' * 60)\n",
            "if best_exp:\n",
            "    print(f\"\\n🏆 Best Model: {best_exp} (val_loss: {best_val_loss:.4f})\")\n",
            "    baseline_val_loss = results['exp001']['val_loss']\n",
            "    improvement = (1 - best_val_loss / baseline_val_loss) * 100\n",
            "    if improvement > 0:\n",
            "        print(f\"✅ {improvement:.1f}% improvement over baseline (exp001)\")\n",
            "    else:\n",
            "        print(f\"⚠️  {-improvement:.1f}% worse than baseline (exp001)\")\n",
            "\n",
            "print('\\n' + '='*60)\n",
            "print('✅ Experiment Complete!')\n",
            "print('='*60)\n",
            "print('\\nGenerated files:')\n",
            "print('  • loss_curves.png - Training/validation loss curves')\n",
            "print('  • final_comparison.png - Final validation loss comparison')\n",
            "print('  • results.json - All experiment metrics')\n",
            "print('\\n🚀 Upload these files to analyze results!')",
        ]),
        ...
        code([
            ...
            "# ============================================================\n",
            "def generate_sample(model, prompt, tok, device, max_tokens=50):\n",
            "    model.eval()\n",
            ...
            "\n",
            "# Load best models and generate samples\n",
            "sample_prompts = [\n",
            "    (\"location\", \"John was in the kitchen. Mary was in the garden. John moved from kitchen to bedroom. Where is John?\"),\n",
            "    (\"inventory\", \"Alex was in the office with apple and book. Alex picked up key. What does Alex have?\"),\n",
            "]\n",
            "\n",
            "samples = {}\n",
            "for exp_id in ['exp001', 'exp003', 'exp005']:  # baseline, SSM+think, decoder\n",
            "    c = next(e for e in EXPS if e['exp_id'] == exp_id)\n",
            "    print(f\"\\n=== Samples from {exp_id} ({c['model']}) ===\")\n",
            "    \n",
            "    # Build model\n",
            "    if c['model']=='baseline':\n",
            "        m = BaselineTransformer(vs=tok.vocab_size, dm=c['dm'], nh=8, nl=4)\n",
            ...
            "        gen = generate_sample(m, prompt, tok, device)\n",
            "        samples[exp_id].append(dict(task=task, prompt=prompt, generated=gen))\n",
            "        print(f\"\\n[{task}] Prompt: {prompt[:60]}...\")\n",
...
print('Architecture: All models produce [batch, seq_len, vocab_size]')
print('LatentSSM: Sequential token processing with periodic thinking')
